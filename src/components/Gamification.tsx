import { Award, ShieldCheck, HelpCircle, Flame, Star, Trophy, Target } from "lucide-react";

const badges = [
  { emoji: "🥇", label: "Pioneer", desc: "First verified filing" },
  { emoji: "🕳️", label: "Pothole Hunter", desc: "10 potholes identified" },
  { emoji: "⚡", label: "City Fixer", desc: "5 community resolutions" },
  { emoji: "🔥", label: "Civic Warrior", desc: "30-day filing streak" },
  { emoji: "💧", label: "Water Guardian", desc: "5 water leak reports" },
  { emoji: "💡", label: "Light Keeper", desc: "10 streetlights fixed" },
  { emoji: "🌳", label: "Green Hero", desc: "5 local park complaints" },
  { emoji: "👑", label: "Ward Guardian", desc: "Top reporter score" },
];

const leaderboards = [
  { rank: "👑", name: "Ravi Kumar", reports: 148, score: 9840, tag: "Ward Guardian" },
  { rank: "🥈", name: "Priya Nair", reports: 121, score: 8210, tag: "City Fixer" },
  { rank: "🥉", name: "Ahmed Khan", reports: 98, score: 7655, tag: "Civic Warrior" },
  { rank: "4️⃣", name: "Sunita Sharma", reports: 87, score: 6490, tag: "Pioneer" },
];
      ...
      <div className="relative z-10 flex flex-col lg:flex-row gap-4 mb-16 items-start lg:items-center">
        ...
        <div className="space-y-3">
          <h2 className="text-3xl sm:text-5xl text-white font-display font-medium tracking-tight">
            CivicScore & Gamification
          </h2>
          <p className="text-neutral-400 text-sm sm:text-base max-w-2xl leading-relaxed font-sans">
            Turn public service into an engaging experience. Report issues, upvote neighbors, verify local resolutions, and earn CivicScore to lock in custom badges and climb the municipal scoreboard.
          </p>
        </div>
      </div>
            ...
            <div className="relative w-32 h-32 flex items-center justify-center bg-black/40 rounded-full border border-white/5">
              <svg className="w-28 h-28 -rotate-90 overflow-visible" viewBox="0 0 100 100">
                ...
                <circle
                  ...
                  strokeWidth="6"
                  strokeDasharray="264"
                  strokeDashoffset="26" // represents 9.0 out of 10
                  strokeLinecap="round"
                />
                <defs>
              ...
              <div className="absolute text-center">
                <div className="text-3xl text-white font-display font-semibold">9.2</div>
                <div className="text-[9px] text-orange-400 uppercase tracking-widest font-semibold font-sans mt-0.5">Rating</div>
              </div>
            </div>
            ...
            <div className="space-y-4 flex-1">
              <div className="bg-white/5 border border-white/5 p-3 rounded-2xl flex items-center justify-between">
                <div>
                  ...
                  <span className="text-white font-medium font-sans">148 reports filed</span>
                </div>
              ...
              <div className="bg-white/5 border border-white/5 p-3 rounded-2xl flex items-center justify-between">
                <div>
                  <span className="text-[10px] text-neutral-500 uppercase tracking-wider block font-sans">Verification count</span>
                  <span className="text-white font-medium font-sans">127 issues solved</span>
                </div>
          ...
          <div className="bg-[#0A0A0A] border border-white/10 rounded-3xl p-6 sm:p-8">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              {badges.map((b, i) => (
                <div
                  key={i}
                  className="flex flex-col items-center gap-1.5 p-3 bg-white/5 border border-white/5 rounded-2xl hover:border-orange-500/20 hover:bg-orange-500/5 transition-all duration-300 group cursor-help text-center"
                  title={b.desc}
                >
                  <span className="text-3xl filter saturate-100 group-hover:scale-110 transition-transform">{b.emoji}</span>
                  <span className="text-xs font-semibold text-neutral-300 font-sans tracking-tight">{b.label}</span>
                  <span className="text-[9px] text-neutral-500 leading-tight font-sans">{b.desc}</span>
                </div>
              ))}
            </div>
          </div>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="text-[10px] text-neutral-500 font-sans">
                        {user.reports} community reports submitted • {user.tag}
                      </div>
                    </div>
                  </div>
